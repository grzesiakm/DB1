...
FROM emp 
JOIN (SELECT MAX(sal) AS maxsal, dname, AVG(deptno) AS deptno 
FROM dept d 
JOIN emp e USING (deptno) 
GROUP BY dname) AS foo USING (deptno) 
WHERE sal=maxsal;
...
SELECT w.ename, t.ename, t.grade 
...
SELECT e.ename, s.ename, e.dname 
...
SELECT DISTINCT least (e.ename, s.ename) AS eename, greatest (e.ename, s.ename) AS sename 
FROM emp e 
...
SELECT * 
FROM (SELECT * FROM emp e WHERE job='MANAGER') AS manager 
...
(SELECT MAX(average), MIN(average) 
FROM (SELECT deptno, AVG(sal) AS average 
FROM emp e 
